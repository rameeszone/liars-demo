...
app.get('/createRoom/', (req, res) => {
    ...
    gameRoom[roomId]["gameStatus"] = "notStarted";
            ...
            if (message.type === "utf8") {
                ...
                switch (response.action) {
                    case "InitialJoin":
                        ...
                        if (response.userId != null && response.roomName != null && gameRoom[response.roomName] != null) {

                            var alreadyJoined = false;
                            gameRoom[response.roomName].forEach((player, index) => {
                                if (player != 0 && player.userId == response.userId) {
                                    ...
                                    connection.country = player.name;
                                    connection.dice = player.dice;
                                    connection.diceLength = player.diceLength;
                                    connection.roomName = player.roomName;
                                    connection.seatId = player.seatId;
                                    connection.rip = player.rip
                                    ...
                                    reply.action = "rejoined";
                                    reply.players = players;
                                    reply.gameStatus = gameRoom[roomName]["gameStatus"];
                                    reply.gameStarted = gameRoom[roomName]["gameStarted"];
                                    reply.displayDice = gameRoom[roomName]["displayDice"];
                                    reply.seatId = connection.seatId;
                                    reply.dice = connection.dice;
                                    connection.sendUTF(JSON.stringify(reply));

                                    var reply = new Object();
                                    reply.action = "NewClientJoined";
                                    player.sendUTF(JSON.stringify(reply));
                                    player.close();

                                    gameRoom[response.roomName][index] = connection;
                                    alreadyJoined = true;
                                }
                            });
                    ...
                    case "Bid":
                        if (connection.seatId == gameRoom[connection.roomName]["currentTurn"]) {
                            ...
                            if (response.bid > gameRoom[connection.roomName]["currentBid"]) {
                                ...
                                gameRoom[connection.roomName]["log"].push({ "Seat ": connection.seatId, "Name ": connection.name, "Bid ": (Math.floor(response.bid / 10)) + " X " + (Math.floor(response.bid % 10)), "Dice ": connection.dice })
                    ...
                    case "Call":
                        if (connection.seatId == gameRoom[connection.roomName]["currentTurn"]) {
                            ...
                            gameRoom[connection.roomName]["log"].push({ "Seat ": connection.seatId, "Name ": connection.name, "Call ": "Called Liar", "Dice ": connection.dice })
...
function startGame_fn(roomName) {
    ...
    gameRoom[roomName]["log"].push({ "Round ": gameRoom[roomName]["gameRound"] })
    for (var i = 0; i < gameRoom[roomName].length; i++) {
        if (gameRoom[roomName][i].rip == false) {
            gameRoom[roomName]["log"].push({ "Seat ": gameRoom[roomName][i].seatId, "Name ": gameRoom[roomName][i].name, "Dice ": gameRoom[roomName][i].dice })
        } else {
            gameRoom[roomName]["log"].push({ "Seat ": gameRoom[roomName][i].seatId, "Name ": gameRoom[roomName][i].name, "Dice ": [], "RIP ": true })
        }
    }
...
function checkWinning_fn(roomName) {
    ...
    gameRoom[roomName]["displayDice"] = 0;
    ...
    if (dicecount >= no) {
        ...
        gameRoom[roomName]["log"].push({ "Seat ": gameRoom[roomName][gameRoom[roomName]["currentTurn"]].seatId, "Name ": gameRoom[roomName][gameRoom[roomName]["currentTurn"]].name, "Status ": "WON : HONEST", "Dice ": gameRoom[roomName][gameRoom[roomName]["currentTurn"]].dice })
    }
...
function sendLogs(roomName) {

    const logsUrl = 'https://gigaroi.com/Gigaroi-Web/api/customer/game-logs';
    const data = { "roomid": roomName, "log": JSON.stringify(gameRoom[roomName]["log"]) };

    console.log("losserUrl data type : ", data)

    // Make the POST request using axios
    axios.post(logsUrl, data)
        .then(response => {
           // console.log('log sending response ', response); // Handle the response data
            if (response.data.success == true) {

            }
        })
        .catch(error => {
            console.error('log sending error ', error); // Handle any errors that occur
        });
